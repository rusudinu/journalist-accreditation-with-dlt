 ...
 */
export const asLocalhost = env.get('AS_LOCAL_HOST').default('true').example('true').asBoolStrict();
 ...
 */
export const org1ApiKey = env.get('ORG1_APIKEY').required().example('123').asString();

/**
 * API key for Org2
 * Specify this API key with the X-Api-Key header to use the Org2 connection profile and credentials
 */
export const org2ApiKey = env.get('ORG2_APIKEY').required().example('456').asString();
